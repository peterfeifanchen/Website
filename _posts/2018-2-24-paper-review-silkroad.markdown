...
the high computate overhead incurred during load balancing. The paper proposes to 
accomplish L4 load-balancing using programmable ASIC switches sitting at TOR instead, 
where it can be accomplished with two orders of magnitude in power and cost savings. 
...
action table to perform (e.g., modify IP address). Overall, there is an ingress 
...
how network processing maps to hardware bits.
...
techniques as will be described later, it is now conceivable to store a complete set 
of the millions of L4 enries required for L4 load balancing at point-of-prescence (PoPs
/front-end) and back-end aggregate switches. 
...
the ConnTable would not be updated with such a entry until a later time. This window 
results in potential for PCC violation if VIPTable was to change to 
VIPTable<sub>i+1</sub> and subsequent packets of the flow may miss in ConnTable 
...
filter to batch a sequence of these new events and remove duplicates. The CPU simulates
cuckoo hashing in order to determine where to place the newly inserted ConnTable entry
in its downcall back into the ASIC. The delay between triggering the insertion and 
the udpating ASIC SRAM (the path is intermediated by CPU) results in PCC violations
which this paper contributes to solving, by utilizing transactional memories which 
insures statistics updated by a preceding packet are seen by the next packet. 
...
function. This solution seem a bit unnecessary since the false positive rate is so 
low and the downside to false positive matches is a slightly skewed load balance. 
Further this solution would not work for UDP and presents no solution when successive 
collisions occur. 
...
first triggers the TransitTable and the first miss from TransitTable triggers the 
LearnTable flush. Then the set that was flushed would consist of all the entries that 
...
in transit table are inserted in connTable. Again, the paper elides how we know that 
all entries have been inserted. Again I believe there must be a mechanism to flush 
the LearnTable once "Execute" finishes. Like the "Request" stage, the set flushed 
...
While the paper states that a key insight to be able to use a bloomfilter is that at 
...
Evaluation was done with a simulator using real traffic traces from a large web 
service provider for a ToR switch in each kind of cluster (PoP, Frontend, Backend). 
